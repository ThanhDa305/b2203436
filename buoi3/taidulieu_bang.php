...
// Cách 4: Hiển thị dữ liệu với phương thức fetch_row
$result = $conn->query($sql);

echo "<h3>Dữ liệu hiển thị với fetch_row</h3>";
echo "<table border=1><tr><th>ID</th><th>Hoten</th><th>email</th><th>ngaysinh</th></tr>";

while($row = $result->fetch_row()) {
    // Dữ liệu là một mảng chỉ số số nguyên
    $date = date_create($row[3]);
    echo "<tr><td>" . $row[0] . "</td><td>" . $row[1] . "</td><td>" . $row[2] . "</td><td>" . $date->format('d-m-Y') . "</td></tr>";
}

echo "</table>";
// Cách 5: Hiển thị dữ liệu với phương thức fetch_array
$result = $conn->query($sql);

echo "<h3>Dữ liệu hiển thị với fetch_array</h3>";
echo "<table border=1><tr><th>ID</th><th>Hoten</th><th>email</th><th>ngaysinh</th></tr>";

while($row = $result->fetch_array(MYSQLI_ASSOC)) {
    // Dữ liệu là mảng kết hợp
    $date = date_create($row['Birthday']);
    echo "<tr><td>" . $row['id'] . "</td><td>" . $row['fullname'] . "</td><td>" . $row['email'] . "</td><td>" . $date->format('d-m-Y') . "</td></tr>";
}

echo "</table>";
// Cách 6: Hiển thị dữ liệu với phương thức fetch_object
$result = $conn->query($sql);

echo "<h3>Dữ liệu hiển thị với fetch_object</h3>";
echo "<table border=1><tr><th>ID</th><th>Hoten</th><th>email</th><th>ngaysinh</th></tr>";

while($row = $result->fetch_object()) {
    // Dữ liệu là đối tượng với các thuộc tính là tên cột
    $date = date_create($row->Birthday);
    echo "<tr><td>" . $row->id . "</td><td>" . $row->fullname . "</td><td>" . $row->email . "</td><td>" . $date->format('d-m-Y') . "</td></tr>";
}
